...
static atomic_t mi_reclaim_depth = ATOMIC_INIT(0);
...
{
	return ST_MODE == READ_ONCE(pg_mi_reclaim->page_reclaim.event_type);
}

inline int mi_reclaim_swappiness(void)
{
	return READ_ONCE(pg_mi_reclaim->page_reclaim.reclaim_swappiness);
}
...
static inline void do_reclaim(void)
{
	...
	if (atomic_inc_return(&mi_reclaim_depth) > 1) {
		atomic_dec(&mi_reclaim_depth);
		return;
	}
		...
		struct kobj_attribute *attr, const char *buf, size_t len)
{
	...
	*  More than 50 is number of reclaimed pages */
	if (event) {
		if (ST_MODE == event) {
			spin_lock(&pg_mi_reclaim->page_reclaim.reclaim_lock);
			WRITE_ONCE(pg_mi_reclaim->page_reclaim.event_type, event);
		...
		spin_lock(&pg_mi_reclaim->page_reclaim.reclaim_lock);
		WRITE_ONCE(pg_mi_reclaim->page_reclaim.event_type, CANCEL_ST);
		pg_mi_reclaim->page_reclaim.nr_reclaim = DEFAULT_ONCE_RECLAIM_PAGES;
		spin_unlock(&pg_mi_reclaim->page_reclaim.reclaim_lock);
	}
		...
		struct kobj_attribute *attr, const char *buf, size_t len)
{
	...
	spin_lock(&pg_mi_reclaim->page_reclaim.reclaim_lock);
	WRITE_ONCE(pg_mi_reclaim->page_reclaim.reclaim_swappiness, reclaim_swappiness);
